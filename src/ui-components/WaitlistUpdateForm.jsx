...
export default function WaitlistUpdateForm(props) {
  const {
    id: idProp,
    waitlist,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = waitlistRecord
      ? { ...initialValues, ...waitlistRecord }
      : initialValues;
    setName(cleanValues.name);
    setEmail(cleanValues.email);
    setDate_time(cleanValues.date_time);
    setCompany(cleanValues.company);
    setRole(cleanValues.role);
    setErrors({});
  };
  const [waitlistRecord, setWaitlistRecord] = React.useState(waitlist);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Waitlist, idProp)
        : waitlist;
      setWaitlistRecord(record);
    };
    queryData();
  }, [idProp, waitlist]);
  React.useEffect(resetStateValues, [waitlistRecord]);
  const validations = {
    name: [],
    email: [{ type: "Email" }],
    date_time: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Waitlist.copyOf(waitlistRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "WaitlistUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
